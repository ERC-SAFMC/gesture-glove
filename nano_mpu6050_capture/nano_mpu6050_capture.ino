...
void loop() {
    ...
    if (Serial.available() > 0) {
        ...
        if (incomingChar == 's') {
            // 's' received, reset data count and set flag to start printing
            dataCount = 0;
            printData = true;
        }
        Serial.println(F("---------------------------------------------------"));
        Serial.println(F("aX,aY,aZ,gX,gY,gZ,yaw,pitch,roll"));
    }
    #else
    printData = true;
    #endif
    ...
    if (printData && mpu.dmpGetCurrentFIFOPacket(fifoBuffer)) {
        ...
        #ifdef SAMPLING_MODE
        dataCount++;
        if (dataCount >= MAX_DATA_TO_PRINT) {
            printData = false;  // Stop printing data
        }
        #endif
